...
void resizeTable(struct tableBuilder *b, size_t width, size_t height)
{
    if (width >= b->width)
    {
        if (width != b->width)
...
struct table convertToTable(struct tableBuilder b)
{
    ...
    t.inputValues = malloc(sizeof(char **) * t.inputCount);
    ...
    t.targetValues = malloc(sizeof(char **) * t.targetCount);
    ...
    t.actualValues = malloc(sizeof(char **) * t.actualCount);
...
void cleanupTable(struct table t)
{
    size_t i, j;
    ...
    for (i = 0; i < t.inputCount; i++)
    {
        ...
        for (j = 0; j < t.inputValCount; j++)
        {
            free(t.inputValues[i][j]);
        }
        free(t.inputValues[i]);
    }
    free(t.inputVars);
    free(t.inputValues);
    ...
    for (i = 0; i < t.targetCount; i++)
    {
        ...
        for (j = 0; j < t.targetValCount; j++)
        {
            free(t.targetValues[i][j]);
        }
        free(t.targetValues[i]);
    }
    free(t.targetVars);
    free(t.targetValues);

    for (i = 0; i < t.actualCount; i++)
    {
        free(t.actualVars[i]);
        for (j = 0; j < t.actualValCount; j++)
        {
            free(t.actualValues[i][j]);
        }
        free(t.actualValues[i]);
    }
    free(t.actualVars);
    free(t.actualValues);
}
