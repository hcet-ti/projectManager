...
public class StringList {
    ...
    /**
     * Splits string {@code _source} around matches of the given REGULAR
     * EXPRESSION(!) {@code _by}. Trailing empty strings are not included in the
     * resulting StringList. The string "boo:and:foo", for example, yields the
     * following results with these expressions:<br/>
     * <table>
     * <tbody>
    ...
    /**
     * Splits string {@code _source} around the first match of the given REGULAR
     * EXPRESSION(!) {@code _by}. A trailing empty string is not included in the
     * resulting StringList. The string "boo:and:foo", for example, yields the
     * following results with these expressions:<br/>
     * <table>
     * <tbody>
    ...
    /**
     * Splits all strings contained in {@code _source} around matches of the
    ...
    /**
     * Splits the string {@code _source} around occurrences of delimiter string
     * {@code _by} and returns a new StringList consisting of the split parts
     * and the separating delimiters in order of occurrence.<br/>
...
//		return found;
    ...
    /**
     * Concatenates all elements, putting the _separator string between
     * them.<br/>
    ...
    public String concatenate(String _separator, int _start, int _end) {
        ...
        for (int i = Math.min(_start, count()); i < Math.min(_end, count()); i++) {
            ...
            //else
            else if (_separator != null) // END KGU#425 2019-10-02
            {
    ...
    /**
     * Multi-line text formed from the list elements as lines (actually the same
     * as this.concatenate("\n"))
     *
...
//        return ret;
     ...
     */
    public int removeAll(String _string) // START KGU#375 2017-04-04: For regularity, new method to remove case-independently
    //    {
    //    	int nRemoved = 0;
    //    	int i = 0;
    //    	while (i < count())
    ...
    /**
     * Removes all elements being exactly or case-insensitively equal to the
     * given string _string
     *
     * @param _string - the searched string
     * @param _matchCase - if the string is be compared exactly (or
     * case-ignoringly)
     * @return number of deletions
     */
    ...
    /**
     * Removes all subsequences being equal to _subList, either
     * case-independently or not, according to the _matchCase argument
     *
...
//    	return nReplaced;
     ...
     * that this.concatenate().trim() and this.trim().concatenate() produce the
     * same result.<br/>
     * Note that the StringList itself is bound to be modified, not a copy of
     * this!
     *
