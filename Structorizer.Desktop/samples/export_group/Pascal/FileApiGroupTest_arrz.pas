...
UNIT FileApiGroupTest_arrz;
{ Generated by Structorizer 3.32-01 }

{ Copyright (C) 2021-10-04 Kay Gürtzig }
{ License: GPLv3-link }
{
...
function generateRandomNumberFile(fileName: string; count: Longint; minVal: Longint; maxVal: Longint): Longint;
...
begin
  assign(fileNo, fileName); rewrite(fileNo);
  { TODO: Consider replacing this file test using IOResult! }
  if (fileNo <= 0) then
  begin
    generateRandomNumberFile := fileNo;
    exit;
  end;
  try
    try
  ...
  generateRandomNumberFile := count;
...
uses FileApiGroupTest_arrz,StructorizerFileAPI;
