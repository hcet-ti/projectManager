...
\tcc{ Writes a text file with name fileName, consisting of count lines, each containing }
\tcc{ a random number in the range from minVal to maxVal (both inclusive). }
\tcc{ Returns either the number of written number if all went well or some potential }
\tcc{ error code if something went wrong. }
\Func{\FuncSty{generateRandomNumberFile(}\ArgSty{fileName, count, minVal, maxVal}\FuncSty{)}:integer}{
\KwData{\(fileName\): string}
\KwData{\(count\): int}
\KwData{\(minVal\): integer}
\KwData{\(maxVal\): integer}
\KwResult{integer}
  \(fileNo\leftarrow{}fileCreate(fileName)\)\;
  \If{\(fileNo\leq\ 0\)}{
    \Return{\(fileNo\)};
  }
  \Try{
    \For{\(k\leftarrow 1\) \KwTo \(count\) \textbf{by} \(1\)}{
      \(number\leftarrow{}minVal+random(maxVal-minVal+1)\)\;
