...
public class SORTING_TEST_MAIN {
	...
	public static void main(String[] args) {
		...
		class Worker465fbb06_0 implements Callable<Object[]> {
			private int[] values1;
		...
		class Worker465fbb06_1 implements Callable<Object[]> {
			private int[] values2;
			private ??? elementCount;
			public Worker465fbb06_1(int[] values2, ??? elementCount) {
		...
		class Worker465fbb06_2 implements Callable<Object[]> {
			private int[] values3;
			public Worker465fbb06_2(int[] values3) {
		...
		try {
			...
			// ----------------- START THREAD 0 ----------------- 
			Future<Object[]> future465fbb06_0 = pool.submit( new Worker465fbb06_0(values1) );
		
			// ----------------- START THREAD 1 ----------------- 
			Future<Object[]> future465fbb06_1 = pool.submit( new Worker465fbb06_1(values2, elementCount) );
		
			// ----------------- START THREAD 2 ----------------- 
			Future<Object[]> future465fbb06_2 = pool.submit( new Worker465fbb06_2(values3) );
		
			// ----------------- AWAIT THREAD 0 ----------------- 
			future465fbb06_0.get();
			// ----------------- AWAIT THREAD 1 ----------------- 
			future465fbb06_1.get();
			// ----------------- AWAIT THREAD 2 ----------------- 
			future465fbb06_2.get();
			pool.shutdown();
		}
	...
	private static void quickSort(??? values, ??? start, ??? stop) {
		// =========== START PARALLEL WORKER DEFINITIONS ============ 
		class Worker98a2d384_0 implements Callable<Object[]> {
			private ??? values;
			private ??? start;
			private ??? p;
			public Worker98a2d384_0(??? values, ??? start, ??? p) {
				this.values = values;
				this.start = start;
				this.p = p;
			}
		...
		class Worker98a2d384_1 implements Callable<Object[]> {
			private ??? values;
			private ??? p;
			private ??? stop;
			public Worker98a2d384_1(??? values, ??? p, ??? stop) {
				this.values = values;
				this.p = p;
				this.stop = stop;
			}
		...
		if (stop >= start + 2) {
			...
			try {
				...
				// ----------------- START THREAD 0 ----------------- 
				Future<Object[]> future98a2d384_0 = pool.submit( new Worker98a2d384_0(values, start, p) );
			
				// ----------------- START THREAD 1 ----------------- 
				Future<Object[]> future98a2d384_1 = pool.submit( new Worker98a2d384_1(values, p, stop) );
			
				// ----------------- AWAIT THREAD 0 ----------------- 
				future98a2d384_0.get();
				// ----------------- AWAIT THREAD 1 ----------------- 
				future98a2d384_1.get();
				pool.shutdown();
			}
			catch (Exception ex) { System.err.println(ex.getMessage()); ex.printStackTrace(); }
			// ========================================================== 
