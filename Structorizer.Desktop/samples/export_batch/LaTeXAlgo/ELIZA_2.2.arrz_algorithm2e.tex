...
\tcc{ Checks whether newInput has occurred among the last }
\tcc{ length(history) - 1 input strings and updates the history }
\Func{\FuncSty{checkRepetition(}\ArgSty{history, newInput}\FuncSty{)}:boolean}{
\KwData{\(history\): array}
\KwData{\(newInput\): string}
\KwResult{boolean}
  \(hasOccurred\leftarrow{}false\)\;
  \If{\(length(newInput)>4\)}{
    \(currentIndex\leftarrow{}history[0];\)\;
    \For{\(i\leftarrow 1\) \KwTo \(length(history)-1\) \textbf{by} \(1\)}{
      \If{\(newInput=history[i]\)}{
        \(hasOccurred\leftarrow{}true\)\;
      }
    }
    \(history[history[0]+1]\leftarrow{}newInput\)\;
    \(history[0]\leftarrow{}(history[0]+1)\bmod(length(history)-1)\)\;
  }
  \Return{\(hasOccurred\)}
}
\end{function}
...
\Prog{\FuncSty{ELIZA}}{
  ...
  \Output{\(\)"{}*\ Version:\ 2.2\ (2019-03-28)"{}\(\)}
  \Output{\(\)"{}**********************************"{}\(\)}
  \tcc{ Stores the last five inputs of the user in a ring buffer, }
  \tcc{ the first element is the current insertion index }
  \(history\leftarrow{}\{0,\)"{}"{}\(,\)"{}"{}\(,\)"{}"{}\(,\)"{}"{}\(,\)"{}"{}\(\}\)\;
