...
\COMMENT{ Checks whether newInput has occurred among the last }\\
\COMMENT{ length(history) - 1 input strings and updates the history }\\
\PROCEDURE{checkRepetition}{history, newInput}
  hasOccurred\gets\FALSE\\
  \IF length(newInput)>4 \THEN
  \BEGIN
    currentIndex\gets\ history[0];\\
    \FOR i \gets 1 \TO length(history)-1  \DO
      \IF newInput=history[i] \THEN
        hasOccurred\gets\TRUE\\
    history[history[0]+1]\gets\ newInput\\
    history[0]\gets(history[0]+1)\bmod(length(history)-1)\\
  \END\\
  \RETURN{hasOccurred}\\
\ENDPROCEDURE
\end{pseudocode}
...
\MAIN
  ...
  \COMMENT{ the first element is the current insertion index }\\
  history\gets\{0,\)"{}"{}\(,\)"{}"{}\(,\)"{}"{}\(,\)"{}"{}\(,\)"{}"{}\(\}\\
