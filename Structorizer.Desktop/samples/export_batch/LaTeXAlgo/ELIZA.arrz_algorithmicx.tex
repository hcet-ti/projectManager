...
\Function{conjugateStrings}{sentence, key, keyPos, flexions}
  ...
  \EndFor
  \ForAll{\(str \in \{\)"{}\ \ "{}\(,\)"{}\textbar{}"{}\(\}\)}\Comment{Eliminate multiple spaces (replaced by single ones) and vertical bars}
...
\State \Comment{ Returns an array of pairs of mutually substitutable words }
\State \Comment{ The second word may contain a '{}\textbar{}'{} in order to prevent an inverse }
\State \Comment{ replacement. }
  \Decl{Result type:}
    \State array of array[0..1] of string
  \EndDecl
...
\State \Comment{ 2022-01-11 Measures against substition inversions a -\textgreater b -\textgreater a in conjugateStrings, reflexions revised. }
  \Decl{Includes:}
  ...
  \State \(\)print\((\)"{}*\ Version:\ 2.4\ (2022-01-11)"{}\()\)
