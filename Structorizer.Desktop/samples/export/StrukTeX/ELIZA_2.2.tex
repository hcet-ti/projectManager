...
\begin{struktogramm}(153,107)[adjustSpelling]
  \assign{%
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(165,63)[checkGoodBye]
  \assign{%
    \begin{declaration}[Parameters:]
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(133,81)[checkRepetition]
  \assign{%
    \begin{declaration}[Parameters:]
  ...
  \ifthenelse{10}{2}{\(length(newInput)\ >\ 4\)}{T}{F}
    \assign{\(currentIndex\ \gets\ history[0];\)}
    \while{\pKey{for}\(i\ \gets\ 1\)\pKey{to}\(length(history)-1\)}
      ...
      \ifthenelse{3}{1}{\(newInput\ =\ history[i]\)}{T}{F}
        \assign{\(hasOccurred\ \gets\ true\)}
      \change
      \ifend
    \whileend
    \assign{\(history[history[0]+1]\ \gets\ newInput\)}
    \assign{\(history[0]\ \gets\ (history[0]\ +\ 1)\ \bmod\ (length(history)\ -\ 1)\)}
  \change
  \ifend
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(238,96)[conjugateStrings]
  \assign{%
    \begin{declaration}[Parameters:]
      \description{\pVar{sentence}}{type: \(string\)}
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(199,90)[findKeyword]
  \assign{%
    \begin{declaration}[Parameters:]
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(214,64)[normalizeInput]
  \assign{%
    \begin{declaration}[Parameters:]
      \description{\pVar{sentence}}{type: \(string\)}
    \end{declaration}
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(170,29)[setupGoodByePhrases]
  \assign{%
    \begin{declaration}[Result type:]
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(113,145)[setupKeywords]
  \assign{%
    \begin{declaration}[Result type:]
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(102,85)[setupReflexions]
  \assign{%
    \begin{declaration}[Result type:]
      \description{array of array[0..1] of string}{}
    \end{declaration}
  ...
  \assign{\(reflexions[4]\ \gets\ \{\)"{}\ i\textbackslash{}'{}ve\ "{}\(,\ \)"{}\ you\textbackslash{}'{}ve\ "{}\(\}\)}
  \assign{\(reflexions[5]\ \gets\ \{\)"{}\ i\textbackslash{}'{}m\ "{}\(,\ \)"{}\ you\textbackslash{}'{}re\ "{}\(\}\)}
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(140,535)[setupReplies]
  \assign{%
    \begin{declaration}[Result type:]
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(258,318)[ELIZA]
  \assign{%
    \begin{declaration}[Requires:]
      \description{KeyMapEntry}{}
    \end{declaration}
  ...
  \assign{\(\)\pKey{OUTPUT}\(\ \)"{}*\ Version:\ 2.2\ (2019-03-28)"{}\(\)}
  \assign{\(\)\pKey{OUTPUT}\(\ \)"{}**********************************"{}\(\)}
  \assign{\(history\ \gets\ \{0,\ \)"{}"{}\(,\ \)"{}"{}\(,\ \)"{}"{}\(,\ \)"{}"{}\(,\ \)"{}"{}\(\}\)}
...
% TODO: Tune the width and height argument if necessary!
\begin{struktogramm}(129,23)[KeyMapEntry]
  \assign{%
