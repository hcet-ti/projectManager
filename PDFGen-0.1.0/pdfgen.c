...
#include "../kat-master/include/highlight.h"
#include "../tiny-regex-c-master/re.h"
#include "../mregexp-master/mregexp.h"
/* custom code end */
...
char *strrep(const char *orig, char *rep, char *with) {
...
float cm2pt(double cm)
{
    return 28.3465 * cm;
}

void pdf_add_text_right(struct pdf_doc *pdf, struct pdf_object *page, char const text[], const float size, float x, float y, uint32_t colour)
{
    float textWidth;
    pdf_get_font_text_width(pdf, pdf->current_font->font.name, text, size, &textWidth);
    pdf_add_text(pdf, page, text, size, pdf->width - textWidth - x, y, colour);
}

void pdf_add_text_center(struct pdf_doc *pdf, struct pdf_object *page, char const text[], const float size, float x, float y, uint32_t colour)
{
    float textWidth;
    pdf_get_font_text_width(pdf, pdf->current_font->font.name, text, size, &textWidth);
    pdf_add_text(pdf, page, text, size, (pdf->width - textWidth) / 2 - x, y, colour);
}

float pdf_top(struct pdf_doc *pdf, float y)
{
    return pdf->height - y;
}

void pdf_append_labeled_page(struct pdf_doc *pdf, const char *header1, const char *header2, const char *footer1, const char *footer2, int *pageCounter)
{
    char font[64] = "";

    strcpy(font, pdf->current_font->font.name);
    pdf_set_font(pdf, "Helvetica");
    pdf_append_page(pdf);
    ++*pageCounter;

    // header
    float cursorY = pdf_top(pdf, 10 + cm2pt(1.25));
    float cursorX = cm2pt(2.5);
    pdf_add_text(pdf, NULL, header1, 10, cursorX, cursorY, PDF_BLACK);
    pdf_add_text_right(pdf, NULL, header2, 10, cursorX, cursorY, PDF_BLACK);

    // footer left
    cursorY = cm2pt(1.25);
    pdf_add_text(pdf, NULL, footer1, 10, cursorX, cursorY, PDF_BLACK);

    // footer center
    pdf_add_text_center(pdf, NULL, footer2, 10, 0, cursorY, PDF_BLACK);

    pdf_set_font(pdf, font);
}

struct theme createTheme(uint32_t preproc, uint32_t types, uint32_t keywords, uint32_t number, uint32_t string, uint32_t comment, uint32_t function, uint32_t symbol)
{
    return (struct theme) {
        preproc, 
        types, 
...
static uint32_t stripColour(char *text, struct theme palette)
{
    uint32_t colour = PDF_BLACK;

    if (!strcmp(text, "\033[38;5;63m"))
    { // cornflower blue
        colour = palette.preproc;
    } else if (!strcmp(text, "\033[38;5;83m"))
    { // screamin' green
        colour = palette.types;
    } else if (!strcmp(text, "\033[38;5;227m"))
    { // laser flamingo
        colour = palette.keywords;
    } else if (!strcmp(text, "\033[38;5;214m"))
    { // yellow sea
        colour = palette.number;
    } else if (!strcmp(text, "\033[38;5;207m"))
    { // pink flamingo
        colour = palette.string;
    } else if (!strcmp(text, "\033[38;5;102m"))
    { // gray
        colour = palette.comment;
    } else if (!strcmp(text, "\033[38;5;193m"))
    { // reef
        colour = palette.function;
    } else if (!strcmp(text, "\033[38;5;101m"))
    { // clay creek
        colour = palette.symbol;
    }

    return colour;
}

void pdf_add_code(struct pdf_doc *pdf, const char *path, const float size, float x, float y, const char *header1, const char *header2, const char *footer1, const char *footer2, int *pageCounter, float *endY, struct theme palette)
{
    FILE *fp;
    fp = fopen(path, "r");
    char buff[255];
    ...
    uint32_t colour = PDF_BLACK;
    MRegexp *re = mregexp_compile("\033\\[(\\d|;)+?m");
    MRegexpMatch m;
    float textWidth = 0;
    int xCursor = 0;
    int xOffset = 0;
    ...
    {
        printf("Unable to initialize the highlight!\n");
    ...
    for (int j = 0; fgets(buff, 255, fp) != NULL; j++)
    {
        if (y - j * size * 1.5 < cm2pt(2.5))
        {
            pdf_append_labeled_page(pdf, header1, header2, footer1, footer2, pageCounter);
            pdf_set_font(pdf, "Courier-Bold");
            y = pdf_top(pdf, size + cm2pt(2.5));
            j = 0;
        }
        line = highlight_line(buff, NULL, 0);

        xOffset = 0;
        for (int i = 0; mregexp_match(re, &line[i], &m); i += m.match_end)
        {
            //printf("(%3llu - %3llu) Match\n", m.match_begin, m.match_end);
            strncpy(selection, &line[i], m.match_begin);
            selection[m.match_begin] = '\0';
            xCursor = x + xOffset;
            pdf_add_text(pdf, NULL, selection, size, xCursor, y - j * size * 1.5, colour);
            pdf_get_font_text_width(pdf, "Courier-Bold", selection, size, &textWidth);
            xOffset += textWidth;
            strncpy(colourString, &line[i + m.match_begin], m.match_end - m.match_begin);
            colourString[m.match_end - m.match_begin] = '\0';
            colour = stripColour(colourString, palette);
        }
...
static int strcnt(char *str, char src)
{
    int counter = 0;
...
static void create_image_from_text(char *text, const char *dest)
{
    MagickWand *wand = NULL;
    char *caption = (char *)malloc(strlen("caption:") + strlen(text) + 1);
    ...
    //make the image
    MagickSetSize(wand, 1600, 40 + 40 * strcnt(text, '\n'));
    MagickSetOption(wand, "stroke", "BLACK");
    MagickSetOption(wand, "strokewidth", "0");
    MagickSetOption(wand, "fill", "BLACK");
    MagickSetOption(wand, "font", "Consolas");
    MagickSetOption(wand, "family", "Consolas");
    MagickSetGravity(wand, WestGravity);
    strcpy(caption, "caption:");
    strcat(caption, text);
    MagickReadImage(wand, caption);
...
int ppm_get_x(struct PPMImage *ppm)
{
    return ppm->x;
}

int ppm_get_y(struct PPMImage *ppm)
{
    return ppm->y;
}
 
struct PPMImage* ppm_load(const char *fname)
{
    FILE *fp;
    struct PPMImage *img = malloc(sizeof(*img));   //!! allocate space!
    int d;
    ...
    if (buff[0] =='P' && (buff[1] == '2' || buff[1] == '3'|| buff[1] == '4'|| buff[1] == '5'|| buff[1] == '6'))
    {
...
void pdf_move_cursor(struct pdf_doc *pdf, float *cursorY, float mandatoryOffset, float optionalOffset, int *pageCounter, const char *header1, const char *header2, const char *footer1, const char *footer2)
{
    if ((*cursorY - (mandatoryOffset + optionalOffset)) < cm2pt(2.5))
    {
        pdf_append_labeled_page(pdf, header1, header2, footer1, footer2, pageCounter);
        *cursorY = pdf_top(pdf, mandatoryOffset + cm2pt(2.5));
    }
    else
    {
        *cursorY -= (mandatoryOffset + optionalOffset);
    }
}

void pdf_add_exe(struct pdf_doc *pdf, const char *path, const char *comp, const char *logLocation, float x, float *yCursor, const char *header1, const char *header2, const char *footer1, const char *footer2, int *pageCounter)
{
    float imageHeight = 0.0f;
    long length = 0L;
    char cmd[_MAX_PATH * 3 + 19] = "";
    char *buff;
    char *tmp;
    FILE *fp = NULL;
    ...
    // compile
    printf("Compiling source file, this might take a while...\n");
    sprintf(cmd, "\"\"%s\" \"%s\" >> \"%s\" 2>&1\"", comp, path, logLocation);
    if (system(cmd) != 0)
    {
        printf("A problem occured during compilation, aborting...\n");
        system("pause>nul|set/p = ");
        exit(EXIT_FAILURE);
    }

    // execute
    tmp = strrep(comp, "compiler.exe", "program.exe");
    sprintf(cmd, "\"\"%s\"\"", tmp);
    if (tmp == NULL || system(cmd) != 0) // right OR will be abandoned, if left OR is already true.
    {
        printf("A problem occured during execution, aborting...\n");
        system("pause>nul|set/p = ");
        exit(EXIT_FAILURE);
    }
    free(tmp);
    ...
    // open output
    tmp = strrep(comp, "compiler.exe", "output.txt");
    if (tmp == NULL || (fp = fopen(tmp, "r")) == NULL)
    {
        printf("A problem occured during the reading of the output, aborting...\n");
        system("pause>nul|set/p = ");
        exit(EXIT_FAILURE);
    }
    free(tmp);
    ...
    // create screenshot
    tmp = strrep(comp, "compiler.exe", "img/output.jpg");
    fseek(fp, 0, SEEK_END);
    length = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    ...
    printf("Creating output image, this might take a while...\n");
    create_image_from_text(buff, tmp);
    imageHeight = cm2pt(16) * ((float) (40 + 40 * strcnt(buff, '\n')) / 1600);
    pdf_add_image_file(pdf, NULL, x, *yCursor - imageHeight, cm2pt(16), imageHeight, tmp);
    pdf_move_cursor(pdf, yCursor, 0, imageHeight, pageCounter, header1, header2, footer1, footer2);

    free(buff);
    free(tmp);
}

static float text_width(struct pdf_doc *pdf, const char *text, const char *font, const float size)
{
    float textWidth = 0.0f;
    pdf_get_font_text_width(pdf, font, text, size, &textWidth);
    return textWidth;
}

void pdf_add_table(struct pdf_doc *pdf, struct table table, 
                   const float paddingX, const float paddingY, 
                   const float x, float *yCursor, const char *header1, 
                   const char *header2, const char *footer1, 
                   const char *footer2, int *pageCounter, 
                   const float size, const float width)
{
    float maxWidth = 0.0f;
    ...
    // get the width of all the input variables combined
    for (size_t i = 0; i < table.inputCount; i++)
    {
        inputWidth += text_width(pdf, table.inputVars[i], "Helvetica", size) + paddingX * 2;
    }
    inputWidth += 2 * paddingX * (table.inputCount - 1);
    
    // compare the width
    if (text_width(pdf, "input data", "Helvetica", size) + 2 * paddingX > inputWidth)
    {
        inputWidth = text_width(pdf, "input data", "Helvetica", size) + 2 * paddingX;
    }

    // get the width of all the target variables combined
    for (size_t i = 0; i < table.targetCount; i++)
    {
        targetWidth += text_width(pdf, table.targetVars[i], "Helvetica", size) + paddingX * 2;
    }
    targetWidth += 2 * paddingX * (table.targetCount - 1);
    
    // compare the width
    if (text_width(pdf, "output data - target", "Helvetica", size) + 2 * paddingX > targetWidth)
    {
        targetWidth = text_width(pdf, "output data - target", "Helvetica", size) + 2 * paddingX;
    }

    // get the width of all the actual variables combined
    for (size_t i = 0; i < table.actualCount; i++)
    {
        actualWidth += text_width(pdf, table.actualVars[i], "Helvetica", size) + paddingX * 2;
    }
    actualWidth += 2 * paddingX * (table.actualCount - 1);
    
    // compare the width
    if (text_width(pdf, "output data - actual", "Helvetica", size) + 2 * paddingX > actualWidth)
    {
        actualWidth = text_width(pdf, "output data - actual", "Helvetica", size) + 2 * paddingX;
    }

    // figure out the dimensions / resize the widths to fit the page
    maxWidth = pdf_width(pdf) - x * 2;
    totalWidth = inputWidth + targetWidth + actualWidth;
    
    inputWidth = inputWidth / totalWidth * maxWidth;
    targetWidth = targetWidth / totalWidth * maxWidth;
    actualWidth = actualWidth / totalWidth * maxWidth;

    totalWidth = inputWidth + targetWidth + actualWidth;

    // Check if there is enough space
    if (*yCursor - (paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2)) < cm2pt(2.5))
    {
        pdf_move_cursor(pdf, yCursor, 0, paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2), pageCounter, header1, header2, footer1, footer2);
    }

    // start by printing the two sides of the table to the pdf
    pdf_add_line(pdf, NULL, x, *yCursor, x, *yCursor - (paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2)), width, PDF_BLACK);
    pdf_add_line(pdf, NULL, pdf_width(pdf) - x, *yCursor, pdf_width(pdf) - x, *yCursor - (paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2)), width, PDF_BLACK);
    
    // add the lines in between
    for (size_t i = 0; i < table.inputValCount + 3; i++)
    {
        pdf_add_line(pdf, NULL, x, *yCursor - (paddingY * i * 2 + size * i), pdf_width(pdf) - x, *yCursor - (paddingY * i * 2 + size * i), width, PDF_BLACK);
    }
    
    // add the lines between the titles
    pdf_add_line(pdf, NULL, x + maxWidth * inputWidth / totalWidth, *yCursor, x + maxWidth * inputWidth / totalWidth, *yCursor - (paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2)), width, PDF_BLACK);
    pdf_add_line(pdf, NULL, x + maxWidth * (targetWidth + inputWidth) / totalWidth, *yCursor, x + maxWidth * (targetWidth + inputWidth) / totalWidth, *yCursor - (paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2)), width, PDF_BLACK);

    // add the lines between the input values
    for (size_t i = 1; i < table.inputCount; i++)
    {
        pdf_add_line(pdf, NULL, x + inputWidth / table.inputCount * i, *yCursor - (paddingY * 2 + size), x + inputWidth / table.inputCount * i, *yCursor - (paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2)), width, PDF_BLACK);
    }
    
    // add the lines between the target values
    for (size_t i = 1; i < table.targetCount; i++)
    {
        pdf_add_line(pdf, NULL, x + inputWidth + targetWidth / table.targetCount * i, *yCursor - (paddingY * 2 + size), x + inputWidth + targetWidth / table.targetCount * i, *yCursor - (paddingY * (table.targetValCount + 2) * 2 + size * (table.targetValCount + 2)), width, PDF_BLACK);
    }
    
    // add the lines between the actual values
    for (size_t i = 1; i < table.actualCount; i++)
    {
        pdf_add_line(pdf, NULL, x + inputWidth + targetWidth + actualWidth / table.actualCount * i, *yCursor - (paddingY * 2 + size), x + inputWidth + targetWidth + actualWidth / table.actualCount * i, *yCursor - (paddingY * (table.actualValCount + 2) * 2 + size * (table.actualValCount + 2)), width, PDF_BLACK);
    }
    
    // add the title text
    pdf_set_font(pdf, "Helvetica");
    pdf_add_text(pdf, NULL, "input data", size, x + (inputWidth - text_width(pdf, "input data", "Helvetica", size)) / 2, *yCursor - size - paddingY / 2, PDF_BLACK);
    pdf_add_text(pdf, NULL, "output data - target", size, x + inputWidth + (targetWidth - text_width(pdf, "output data - target", "Helvetica", size)) / 2, *yCursor - size - paddingY / 2, PDF_RGB(112, 173, 71));
    pdf_add_text(pdf, NULL, "output data - actual", size, x + inputWidth + targetWidth + (actualWidth - text_width(pdf, "output data - actual", "Helvetica", size)) / 2, *yCursor - size - paddingY / 2, PDF_RGB(0, 176, 240));

    // add the input values and names
    for (size_t i = 0; i < table.inputCount; i++)
    {
        for (size_t j = 0; j < table.inputValCount; j++)
        {
            pdf_add_text(pdf, NULL, table.inputValues[i][j], size, x + inputWidth / table.inputCount * i + (inputWidth / table.inputCount - text_width(pdf, table.inputValues[i][j], "Helvetica", size)) / 2, *yCursor - (size + paddingY) * (3 + j) - paddingY * (j + 1) - paddingY / 2, PDF_BLACK);
        }
        pdf_add_text(pdf, NULL, table.inputVars[i], size, x + inputWidth / table.inputCount * i + (inputWidth / table.inputCount - text_width(pdf, table.inputVars[i], "Helvetica", size)) / 2, *yCursor - (size + paddingY) * 2 - paddingY / 2, PDF_BLACK);
    }

    // add the target values and names
    for (size_t i = 0; i < table.targetCount; i++)
    {
        for (size_t j = 0; j < table.targetValCount; j++)
        {
            pdf_add_text(pdf, NULL, table.targetValues[i][j], size, x + inputWidth + targetWidth / table.targetCount * i + (targetWidth / table.targetCount - text_width(pdf, table.targetValues[i][j], "Helvetica", size)) / 2, *yCursor - (size + paddingY) * (3 + j) - paddingY * (j + 1) - paddingY / 2, PDF_RGB(112, 173, 71));
        }
        pdf_add_text(pdf, NULL, table.targetVars[i], size, x + inputWidth + targetWidth / table.targetCount * i + (targetWidth / table.targetCount - text_width(pdf, table.targetVars[i], "Helvetica", size)) / 2, *yCursor - (size + paddingY) * 2 - paddingY / 2, PDF_RGB(112, 173, 71));
    }

    // add the actual values and names
    for (size_t i = 0; i < table.actualCount; i++)
    {
        for (size_t j = 0; j < table.actualValCount; j++)
        {
            pdf_add_text(pdf, NULL, table.actualValues[i][j], size, x + inputWidth + targetWidth + actualWidth / table.actualCount * i + (actualWidth / table.actualCount - text_width(pdf, table.actualValues[i][j], "Helvetica", size)) / 2, *yCursor - (size + paddingY) * (3 + j) - paddingY * (j + 1) - paddingY / 2, PDF_RGB(0, 176, 240));
        }
        pdf_add_text(pdf, NULL, table.actualVars[i], size, x + inputWidth + targetWidth + actualWidth / table.actualCount * i + (actualWidth / table.actualCount - text_width(pdf, table.actualVars[i], "Helvetica", size)) / 2, *yCursor - (size + paddingY) * 2 - paddingY / 2, PDF_RGB(0, 176, 240));
    }
    
    // set the cursor to the end of the table
    pdf_move_cursor(pdf, yCursor, 0, paddingY * (table.inputValCount + 2) * 2 + size * (table.inputValCount + 2), pageCounter, header1, header2, footer1, footer2);
}

void img_get_size(const char *img, size_t *width, size_t *height)
{
    MagickWand *wand = NULL;
...
char *img_strip_alpha(const char *png, char *dest)
{
    MagickWand *wand = NULL;
    if (dest == NULL)
    {
        dest = malloc(strlen(png) + 1);
        strcpy(dest, png);
        dest[strlen(dest) - 3] = 'j';
        dest[strlen(dest) - 2] = 'p';
        // no need to put the 'g' because it's already in "png"
    }
...
void pdf_add_structogram_by_path(struct pdf_doc *pdf, const char *imgSrcPath, const float x, 
                                 float *yCursor, const char *header1, const char *header2, 
                                 const char *footer1, const char *footer2, int *pageCounter,
                                 const char *imgDstPath)
{
    size_t width = 0;
    size_t height = 0;
    double imgWidth = 0.0;
    ...
    // inform user
    clearscreen();
    printf("Adding the image of the structogram, this might take a while...\n");
    
    // strip transparency, because it isn't supported
    img_strip_alpha(imgSrcPath, imgDstPath);

    // scale the image
    img_get_size(imgDstPath, &width, &height);
    imgWidth = pdf_width(pdf) - 2 * x;
    imgHeight = ((double)height / width) * imgWidth;
    if (*yCursor - imgHeight < cm2pt(2.5))
    {
        if (*yCursor - cm2pt(2.5) > pdf_height(pdf) / 2 - cm2pt(2.5))
        {
            imgHeight = *yCursor - cm2pt(2.5) - 1.0f; // remove 1 because of floating point comparison
            imgWidth = ((double)width / height) * imgHeight;
            if (imgWidth > pdf_width(pdf) - 2 * cm2pt(2.5))
            {
                imgWidth = pdf_width(pdf) - 2 * cm2pt(2.5);
                imgHeight = ((double)height / width) * imgWidth;
            }
        }
        else
        {
            imgHeight = pdf_height(pdf) - cm2pt(2.5) * 2;
            imgWidth = ((double)width / height) * imgHeight;
            if (imgWidth > pdf_width(pdf) - 2 * cm2pt(2.5))
            {
                imgWidth = pdf_width(pdf) - 2 * cm2pt(2.5);
    ...
    pdf_move_cursor(pdf, yCursor, imgHeight, 0, pageCounter, header1, header2, footer1, footer2);

    // add the image
    pdf_add_image_file(pdf, NULL, x, *yCursor, imgWidth, imgHeight, imgDstPath);
}
...
void pdf_structorize(struct pdf_doc *pdf, const char *structorizer, 
                     const char *output, const char *code, const char *logLocation,
                     const char *structFolder, const float x, float *yCursor,
                     ...
                     int *pageCounter)
{
    ...
    double imgWidth = 0.0;
    double imgHeight = 0.0;
    char cmd[16 * _MAX_PATH + 10 + 11 + 2 + /* 42 */ 20 + 3 + 2 + 11 + 3 + 1] = "";
    char *fullOutput = strrep(output, "/", "\\");
    if (fullOutput[strlen(fullOutput) - 1] != '\\')
    {
        // make way for the last slash (that would be a cool album title)
        fullOutput = realloc(fullOutput, strlen(fullOutput) + 2);
        strcat(fullOutput, "\\");
    }

    // inform user
    clearscreen();
    printf("Parsing the source file to structorizer, this might take a while...\n");

    // delete everything in the folder
    sprintf(cmd, "del /F /S /Q \"%s*.*\" >> \"%s\" 2>&1", fullOutput, logLocation);
    system(cmd);
    
    // parse to structorizer
    sprintf(cmd, "\"\"%s\" -p ANSI-C99 \"%s\" -o \"%sstruct\" >> \"%s\" 2>&1\"", structorizer, code, fullOutput, logLocation);
    system(cmd);

    sprintf(cmd, "%sstruct.arr", fullOutput);
    if (exists(cmd))
    {    
        // Use the arranger export PNG function if .arr exists
        sprintf(cmd, "java -cp \"" // 10
            "%s\\bin;" // path + ";"
            "%s\\lib\\swing-layout-1.0.3.jar;" // path + ";"
            "%s\\lib\\freehep-graphics2d-2.1.1.jar;" // path + ";"
            "%s\\lib\\freehep-graphicsio-2.1.1.jar;" // path + ";"
            "%s\\lib\\freehep-graphicsio-svg-2.1.1.jar;" // path + ";"
            "%s\\lib\\freehep-io-2.0.2.jar;" // path + ";"
            "%s\\lib\\freehep-swing-2.0.3.jar;" // path + ";"
            "%s\\lib\\freehep-util-2.0.2.jar;" // path + ";"
            "%s\\lib\\freehep-xml-2.1.1.jar;" // path + ";"
            "%s\\lib\\freehep.jar;" // path + ";"
            "%s\\lib\\AppleJavaExtensions.jar;" // path + ";"
            "%s\\lib\\bsh-2.1.0.jar\" " // path + "\ "(2)
            "\"lu.fisch.structorizer.arranger.Arranger\" " // 42
            "\"%sstruct.arr\" " // path + 3
            "\"%sstruct.png\"" // path + 2
            " >> \"%s\" 2>&1", // 11
            structFolder, structFolder, structFolder, structFolder, structFolder, 
            structFolder, structFolder, structFolder, structFolder, structFolder, 
            structFolder, structFolder, fullOutput, fullOutput, logLocation
        );
    ...
    {
        sprintf(cmd, "%sstruct.nsd", fullOutput);
        // This shouldn't happen, but as a last resort if there is no .arr, try to use the  .nsd
        if (exists(cmd))
        {
            // Normal PNG export function
            // NOTE: will shortly display the GUI, before exiting
            sprintf(cmd, "java -cp \"" // 10
                "%s\\bin;" // path + ";"
                "%s\\lib\\swing-layout-1.0.3.jar;" // path + ";"
                "%s\\lib\\freehep-graphics2d-2.1.1.jar;" // path + ";"
        ...
        {
            // This is bad - there is no .arr or .nsd file, so abort
            printf("An error occurred during the creation of the structogram. Could not find struct.arr or struct.nsd, aborting...\n");
            system("pause>nul|set/p = ");
            exit(EXIT_FAILURE);
        }
    }
    ...
    // inform user
    clearscreen();
    printf("Adding the structogram, this might take a while...\n");

    // strip transparency, because it isn't supported
    snprintf(png, _MAX_PATH, "%sstruct.png", fullOutput);
    jpg = img_strip_alpha(png, NULL);

    // scale the image
    img_get_size(jpg, &width, &height);
    imgWidth = pdf_width(pdf) - 2 * x;
    imgHeight = ((double)height / width) * imgWidth;
    if (*yCursor - imgHeight < cm2pt(2.5))
    {
    ...
    // add the image
    pdf_add_image_file(pdf, NULL, x, *yCursor, imgWidth, imgHeight, jpg);
    
    // free
    free(fullOutput);
}

void pdf_add_page_count(struct pdf_doc *pdf, int *pageCounter)
{
    pdf_set_font(pdf, "Helvetica");
    char buff[255];
    struct pdf_object *page = pdf_find_first_object(pdf, OBJ_page);

    for (int i = 0; i < *pageCounter; i++)
    {
        sprintf(buff, "page %d of %d", i + 1, *pageCounter);
        pdf_add_text_right(pdf, page, buff, 10, cm2pt(2.5), cm2pt(1.25), PDF_BLACK);
        page = page->next;
    }
}

char *checkForMagick(const char *exampleImagePath)
{
    ...
    status = MagickReadImage(magick_wand, exampleImagePath);
...
void releaseErrorMsgMagick(char *msg)
{
    msg = (char *)MagickRelinquishMemory(msg);
}